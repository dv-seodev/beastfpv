      ...
      <div className="container order-success__container">
        ...
        <div className="order-success__main">
          ...
          <div className="order-success__info-block">
            ...
            <div className="order-success__info-row">
              ...
              <span className="order-success__value">{formatDate(order.date_created)}</span>
            </div>
            ...
            {order.shipping_lines && order.shipping_lines.length > 0 && (
              <div className="order-success__info-row">
                ...
                <span className="order-success__value">{order.shipping_lines[0].method_title}</span>
              </div>
            )}
          </div>
              ...
              {order.line_items.map((item) => (
                <div key={item.id} className="order-success__item">
                  ...
                  <div className="order-success__item-info">
                    ...
                    {item.variation_id && <p className="order-success__item-variation">Вариант товара</p>}
                  </div>
                  ...
                  <div className="order-success__item-details">
                    <div className="order-success__item-qty">
                      ...
                      <span className="order-success__item-qty-value">{item.quantity} шт.</span>
                    </div>

                    <div className="order-success__item-price">
                      <span className="order-success__item-price-label">Цена:</span>
                      <span className="order-success__item-price-value">{formatPrice(item.price)} ₽</span>
                    </div>
        ...
        <div className="order-success__totals">
          <div className="order-success__totals-row">
            ...
            <span className="order-success__totals-value">{formatPrice(order.subtotal)} ₽</span>
          </div>
          ...
          {order.shipping_total && parseFloat(order.shipping_total) > 0 && (
            <div className="order-success__totals-row">
              ...
              <span className="order-success__totals-value">+{formatPrice(order.shipping_total)} ₽</span>
            </div>
          )}
          ...
          {order.tax_total && parseFloat(order.tax_total) > 0 && (
            <div className="order-success__totals-row">
              ...
              <span className="order-success__totals-value">+{formatPrice(order.tax_total)} ₽</span>
            </div>
          )}
          ...
          {order.discount_total && parseFloat(order.discount_total) > 0 && (
            <div className="order-success__totals-row order-success__totals-row--discount">
              ...
              <span className="order-success__totals-value">-{formatPrice(order.discount_total)} ₽</span>
            </div>
          )}
        ...
        <div className="order-success__notice">
          ...
          <ul>
            <li>
              ✅ На вашу почту <strong>{order.billing.email}</strong> отправлено подтверждение заказа
            </li>
            <li>📦 Вы получите уведомление о готовности к отправке</li>
            <li>
              🔍 Отследить статус заказа можно по номеру <strong>#{order.number || order.id}</strong>
            </li>

            {/* Конкретные инструкции по способу оплаты */}
            {order.payment_method === "cod" && <li>💵 Оплату необходимо произвести наличными при получении товара</li>}
...
// ✅ Оборачиваем в Suspense для поддержки useSearchParams при билде
const OrderSuccess = () => {
  return (
    <Suspense
      fallback={
