...
const Cart = () => {
  ...
  const { methods: shippingMethods, loading: shippingLoading } =
    useShippingMethods();
  const { methods: paymentMethods, loading: paymentLoading } =
    usePaymentMethods();
  ...
  const cartItemsFromGraphQL =
    graphQLCart?.contents?.nodes?.map((node) => ({
      key: node.key,
      id: node.key,
      ...
      image:
        node.product?.node?.image?.sourceUrl || "/images/product_image.jpg",
  ...
  const displayItems =
    cartItemsFromGraphQL.length > 0 ? cartItemsFromGraphQL : items;

  // ✅ ИСПОЛЬЗУЙ ТОЛЬКО GRAPHQL МЕТОДЫ
  const displayShippingMethods =
    graphQLShippingMethods.length > 0
      ? graphQLShippingMethods
      : shippingMethods;

  const displayPaymentMethods =
    graphQLPaymentMethods.length > 0 ? graphQLPaymentMethods : paymentMethods;
  ...
  useEffect(() => {
    if (displayShippingMethods && displayShippingMethods.length > 0) {
      const methodExists = displayShippingMethods.some(
        (m) => m.id === selectedShipping
      );
      if (!methodExists) {
        console.log(
          "🔄 Инициализируем доставку:",
          displayShippingMethods[0].id
        );
  ...
  useEffect(() => {
    if (displayPaymentMethods && displayPaymentMethods.length > 0) {
      const methodExists = displayPaymentMethods.some(
              ...
              {displayItems.map((item) => (
                <div key={item.key} className="cart__product-item">
                  ...
                  <div className="cart__product-item-inner">
                    <Link
                      className="cart__product-item-link"
          ...
          <div className="cart__right-section">
            <div className="cart__price-info">
              ...
              <div className="cart__price-wrapper">
                ...
                {graphQLCart?.appliedCoupons &&
                  graphQLCart.appliedCoupons.length > 0 && (
                    <div className="cart__price-discount cart__price-underline">
                      ...
                      <span className="cart__price-numb action-price">
                        ...
                        {formatPriceForDisplay(
                          parsePrice(
                            graphQLCart.appliedCoupons[0].discountAmount
                          )
                        )}
                      </span>
                    </div>
                      ...
                      displayShippingMethods.map((method) => (
                        <div key={method.id} className="cart__checkbox-main">
                          ...
                          >
                            {method.title}
                            {method.cost > 0 &&
                              ` (+${formatPriceForDisplay(method.cost)})`}
                          </label>
                        </div>
                      ))
                    ) : (
